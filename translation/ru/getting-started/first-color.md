...
```{lit} C++, Surface Configuration ru
...
```{lit} C++, Describe Surface Configuration ru
...
```{lit} C++, Describe Surface Format ru
...
```{lit} C++, Initialize ru (hidden, replace)
...
```{lit} C++, Main loop content ru (replace)
...
```{lit} C++, GetNextSurfaceTextureView method ru
...
```{lit} C++, Application implementation ru (append, hidden)
...
```{lit} C++, Get the next target texture view ru
// Get the next target texture view
...
```{lit} C++, Private methods ru (insert in {{Application class}} after "bool IsRunning();")
private:
...
```{lit} C++, Get the next surface texture ru
...
```{lit} C++, Get the next surface texture ru (append)
...
```{lit} C++, Create surface texture view ru
...
```{lit} C++, Present the surface onto the window ru
...
```{lit} C++, Present the surface onto the window ru (append)
...
```{lit} C++, Present the surface onto the window ru (replace)
// At the end of the frame
wgpuTextureViewRelease(targetView);
...
```{lit} C++, Use Render Pass ru (hidden)
...
```{lit} C++, Describe the attachment ru
...
```{lit} C++, Describe the attachment ru (append)
...
```{lit} C++, Describe Render Pass ru (append)
