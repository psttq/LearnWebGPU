...
```{lit} CMake, Dependency subdirectories ru (prepend)
if (NOT EMSCRIPTEN)
...
```{lit} CMake, Link libraries ru (replace)
...
```{lit} C++, Create and destroy window ru
// Create the window
...
```{lit} C++, Application implementation ru
...
```{lit} C++, Poll WebGPU Events ru (hidden)
...
```{lit} C++, Main loop content ru(hidden)
```

```{lit} C++, Initialize ru (hidden)
...
```{lit} C++, Open window and get adapter ru (hidden)
// Open window
glfwInit();
glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API); // <-- extra info for glfwCreateWindow
glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE);
...
```{lit} C++, Add device error callback ru (hidden)
...
```{lit} C++, Request device ru (hidden)
// Get device
...
```{lit} C++, Build device descriptor ru (hidden)
...
```{lit} C++, Application attributes ru
GLFWwindow *window;
WGPUDevice device;
WGPUQueue queue;
```
...
```{lit} C++, Main function ru (replace, hidden)
int main() {
	Application app;
...
```{lit} C++, Request adapter ru(replace)
{{Get the surface}}
...
```{lit} CMake, glfw3webgpu subdirectory ru (insert in {{Dependency subdirectories}} after "add_subdirectory(webgpu)")
...
```{lit} C++, Application attributes ru (append)
WGPUSurface surface;
```
````
