...
```{lit} C++, Create Command Encoder ru
...
```{lit} C++, Finish encoding and submit ru
...
```{lit} C++, Test command encoding ru (hidden)
{{Get Queue}}
{{Add queue callback}}
