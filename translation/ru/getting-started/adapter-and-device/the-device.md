...
```{lit} C++, Request device function ru
/**
...
```{lit} C++, file: webgpu-utils.h ru (hidden)
#pragma once
...
```{lit} C++, file: webgpu-utils.cpp ru (hidden)
...
```{lit} C++, Utility functions in main.cpp ru (replace, hidden)
```

```{lit} C++, Includes ru (prepend, hidden)
#include "webgpu-utils.h"
```

```{lit} CMake, Define app target ru (replace, hidden)
...
```{lit} CMake, App source files ru (hidden)
main.cpp
webgpu-utils.h
webgpu-utils.cpp
```
...
```{lit} C++, Create things ru (append, hidden)
{{Request device}}
...
```{lit} C++, Destroy things ru (prepend)
...
```{lit} C++, Destroy things ru (replace)
wgpuDeviceRelease(device);
```
...
```{lit} C++, Build device descriptor ru
...
```{lit} C++, Set device lost callback ru
...
```{lit} C++, Utility functions ru (append)
...
```{lit} C++, Extra device limits ru (hidden)
...
```{lit} C++, Set device lost callback ru (replace)
...
```{lit} C++, Setup device callbacks ru
